...
import type { PostDocument } from "@/types/appwrite";
...
const categoryColors: Record<string, string> = {
  ...
  'field-report': 'bg-primary/10 text-primary',
  'success-story': 'bg-accent/10 text-accent-foreground',
  'project-update': 'bg-blue-100 text-blue-700',
  'volunteer-story': 'bg-purple-100 text-purple-700',
      ...
      <div className="relative container mx-auto px-4 md:px-6">
        ...
        <div className="text-center space-y-6 mb-16">
          ...
          <p className="max-w-3xl mx-auto text-xl text-muted-foreground leading-relaxed animate-fadeInUp animation-delay-100">
            {language === 'ar'
              ? 'تابع آخر أخبارنا وقصص نجاحنا وتحديثات المشاريع'
              : 'Stay updated with our latest news, success stories, and project updates'}
          </p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-16 animate-fadeInUp animation-delay-200">
          {categories.map((category) => (
            <Badge
              key={category}
              className={`px-6 py-2.5 cursor-pointer transition-all duration-300 hover:scale-105 ${
                ...
                return (
                  ...
                  >
                    <div className="relative h-64 overflow-hidden">
                      ...
                      {post.category && (
                        <Badge className={`absolute top-4 right-4 ${categoryColors[post.category] || 'bg-muted'}`}>
        ...
        {recentPosts.length > 0 && (
          <section className="animate-fadeInUp animation-delay-500">
            <h2 className="text-3xl font-headline font-bold mb-8">
              {language === 'ar' ? 'أحدث المقالات' : 'Recent Articles'}
            </h2>
        ...
        {posts.length === 0 && (
          <div className="text-center py-20">
            <p className="text-muted-foreground text-lg">
              {language === 'ar' ? 'لا توجد مقالات متاحة حالياً' : 'No articles available at this time'}
            </p>
          </div>
        )}
          ...
          <Card className="shadow-modern-2xl border-border/50 bg-gradient-to-br from-primary/5 to-accent/5">
            <CardHeader className="text-center pb-6">
              ...
              <CardDescription className="text-base mt-2">
                {language === 'ar'
                  ? 'احصل على آخر الأخبار والتحديثات في بريدك الإلكتروني'
